...
def query_kubernetes_cluster(cluster):
    ...
    try:
        response = request(cluster, '/api/v1/namespaces/openshift-infra/services/heapster/proxy/apis/metrics/v1alpha1/nodes')
        response.raise_for_status()
        data = response.json()
    ...
    try:
        response = request(cluster, '/api/v1/namespaces/openshift-infra/services/heapster/proxy/apis/metrics/v1alpha1/pods')
        response.raise_for_status()
        data = response.json()
        if not data.get('items'):
